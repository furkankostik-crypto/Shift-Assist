...
async function start() {
  ...
  const localPort = await getAvailablePort(5180);
  const localUrl = `http://${localIp}:${localPort}`;

  console.log('\x1b[36m%s\x1b[0m', '\n======================================================');
  console.log('\x1b[1m\x1b[32m%s\x1b[0m', '   📱 SHIFT ASSIST - MOBIL GELİŞTİRME & ÖNİZLEME');
  console.log('\x1b[36m%s\x1b[0m', '======================================================\n');
  console.log(`🚀 Sunucular Port ${localPort} üzerinde başlatılıyor, lütfen bekleyin...\n`);

  // 2. Vite Geliştirme Sunucusunu Başlat
  const isWindows = process.platform === 'win32';
  viteProcess = isWindows
    ? spawn('cmd.exe', ['/c', 'npx', 'vite', '--host', '0.0.0.0', '--port', String(localPort), '--strictPort'], {
        stdio: ['pipe', 'pipe', 'pipe']
      })
    : spawn('npx', ['vite', '--host', '0.0.0.0', '--port', String(localPort), '--strictPort'], {
        stdio: ['pipe', 'pipe', 'pipe']
      });
  ...
  tunnelProcess = spawn(cloudflaredCmd, ['tunnel', '--url', `http://localhost:${localPort}`], {
    stdio: ['pipe', 'pipe', 'pipe']
  });
  ...
  function printDashboard(tunnelUrl) {
    ...
    console.log('   Yerel PC: \x1b[4m\x1b[34mhttp://localhost:' + localPort + '\x1b[0m');
    console.log('   Wi-Fi Ağı: \x1b[4m\x1b[34m' + localUrl + '\x1b[0m');
...
function cleanup() {
  const isWindows = process.platform === 'win32';
  try {
    if (isWindows && viteProcess && viteProcess.pid) {
  ...
  try {
    if (isWindows && tunnelProcess && tunnelProcess.pid) {
